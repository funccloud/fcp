...
import (
	...
	"go.funccloud.dev/fcp/internal/proxy/audit"
	"go.funccloud.dev/fcp/internal/proxy/hooks"
	"go.funccloud.dev/fcp/internal/proxy/logging"
	"go.funccloud.dev/fcp/internal/proxy/metadata"
...
func (p *Proxy) Start(ctx context.Context) error {
	waitCh, listenerStoppedCh, err := p.run(ctx.Done())
	if err != nil {
		return err
	}
	<-waitCh
...
func (p *Proxy) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if metadata.NoImpersonation(req) {
		token := metadata.BearerToken(req)
		req.Header.Add("Authorization", token)
		return p.noAuthClientTransport.RoundTrip(req)
	}

	// Get the impersonation headers from the context.
	impersonationConf := metadata.ImpersonationConfig(req)
...
func (p *Proxy) reviewToken(_ http.ResponseWriter, req *http.Request) bool {
	var remoteAddr string
	req, remoteAddr = metadata.RemoteAddr(req)
