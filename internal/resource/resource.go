...
func CheckOrInstallVersion(ctx context.Context, domain string, k8sClient client.Client, log logr.Logger) error {
	...
	if onKind {
		log.Info("Detected Kind cluster via kindnet daemonset. Recomend for dev environment. Run command `make loadbalancer` to enble load balancer.")
		log.Info("Setting domain to 127.0.0.1.nip.io")
